...
Two separate Node 24.19.0 processes reproduced the same assertion values and
statuses. See the [execution report](../../../conformance/evidence/eu-ai-p1.1-20260907/run-02/report.md),
[raw results](../../../conformance/evidence/eu-ai-p1.1-20260907/run-02/results.json),
and [verification record](../../../conformance/evidence/eu-ai-p1.1-20260907/verification.json).
The copied specification inside each run records the document bytes at run
time, before these evidence links were appended.

Compatibility verification passed the 113 existing Standard tests and registry
lint, the Engine build/typecheck, 38 focused Engine/observer tests, and four
new harness tests. The full Engine suite reported 928 passes, two failures,
and six skips out of 936 tests. Its existing change-allowlist test rejects the
new observer path; the allowlist was not modified. An existing retrieval replay
test returned an operational error, also observed in a filtered rerun; that
filtered run additionally failed the suite's all-rows-consumed teardown hook.
These are retained compatibility limitations, not silently waived checks.
The original source and tests remain unchanged; only new example/test files
and planning/evidence updates were added.
